...
import { matchesTarget, screenPoint } from '../src/windows.js';
import { physicalRegion, fullClientOverlay } from '../src/overlay.js';
import { findTemplate } from '../src/matcher.js';
import type { RawImage } from '../src/matcher.js';
...
test('target conditions all match, and DPI coordinates track moved windows', () => {
  const info = { handle: 1, title: 'Example - editor', process_name: 'example.exe', executable_path: 'C:\\example.exe' };
...
test('matching returns location and yields so cancellation can interrupt scanning', async () => {
  const frame = {data: Buffer.from([0,0,0,0,0,1,2,0,0,3,4,0,0,0,0,0]), info: {width:4,height:4}} as unknown as RawImage;
  const template = {data: Buffer.from([1,2,3,4]), info:{width:2,height:2}} as unknown as RawImage;
  ...
  await assert.rejects(findTemplate({data: Buffer.alloc(40000), info:{width:200,height:200}} as unknown as RawImage, template, 0.99, control));
});
