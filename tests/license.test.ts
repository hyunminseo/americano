...
import { issue } from '../scripts/license-issuer.js';
import { verifyLicense, macHash, LicenseManager, readLimited } from '../src/license.js';
...
const signed = (options: any = {}): any => issue(pair.privateKey, { mac, issuedTo: 'Test user', expiresAt: expiry, now: new Date(now), ...options });
const check = (bytes: any, options: any = {}, key: any = publicKey): any => verifyLicense(bytes, key, { now, macs: [mac], ...options });
...
test('correctly signed wrong product and malformed payload fail', () => {
  const envelope = JSON.parse(signed()); const payload = JSON.parse(Buffer.from(envelope.payload, 'base64').toString());
  function altered(change: any): any { const bytes = Buffer.from(JSON.stringify({ ...payload, ...change })); return Buffer.from(JSON.stringify({ ...envelope, payload: bytes.toString('base64'), signature: crypto.sign(null, bytes, pair.privateKey).toString('base64') })); }
...
test('registration persists, rejects replacement by invalid license, and does not fall back after corruption', async (t: TestContext) => {
...
test('expiry interrupts a paused real run but leaves input-free preview available', async () => {
  ...
  let entered: any; const began = new Promise<void>((resolve) => { entered = resolve; });
  const runner = new MacroRunner({ authorize: async () => { manager.authorize(); }, onState: (state: any) => { if (state.step) entered(); } });
  ...
  assert.equal(runner.state().status, 'ERROR'); assert.match(runner.state().error as string, /만료/);
