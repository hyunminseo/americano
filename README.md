...
Windows와 macOS용 Electron 화면 인식 매크로 편집기입니다. Node.js 24.x / npm 11.x를 사용합니다.

기본 화면을 노드와 연결선으로 구성하는 워크플로우 편집기로 개편했습니다. 이미지 조건 분기, 이미지 클릭/대기, 키보드·텍스트·좌표 입력, 전체 반복, 실행 기록, 실행 취소/다시 실행을 제공합니다. 800×600과 1280×960의 동일한 4:3 콘텐츠는 기준 해상도 정규화로 이미지와 좌표를 공유합니다.

**[사용법 · 해상도 설정 · 플랫폼 지원 · 검증 범위](WORKFLOW_STUDIO.md)**

기존 매크로는 노드로 변환해 불러오며, 암호화 저장소와 오프라인 라이선스, 이미지 포함 .amacro 가져오기/내보내기를 유지합니다. 실 실행에는 라이선스와 OS 권한이 필요합니다. macOS/Windows 실제 입력·설치 검증은 아직 완료되지 않았습니다.

```sh
npm ci
npm test
npm start
npm run build:win   # Windows 호스트
npm run build:mac   # macOS 호스트
```

아래는 기존 Windows 배포 및 라이선스 운영 절차입니다.
...
2. **오버레이 영역 설정**에서 드래그하고 Enter를 누릅니다. Esc는 취소입니다. 영역은 client 영역 기준 96 DPI 좌표로 자동 저장합니다. 드래그 없이 시작하려면 **전체 창 자동 설정**으로 클라이언트 전체를 오버레이로 지정합니다.
...
매크로 편집기는 위에서 아래로 이어지는 워크플로우 방식입니다. 조건의 참·거짓 레인과 반복 컨테이너 안에 동작 노드를 넣고, 노드를 클릭하면 오른쪽 인스펙터에서 편집합니다. 기존 매크로도 워크플로우로 불러올 수 있습니다.

**매크로 내보내기**는 설정과 캡처 이미지를 `.amacro` 파일로 묶습니다. **매크로 가져오기** 후 대상 창을 확인하고 오버레이를 다시 지정하세요. 영역 안의 클릭은 새 오버레이 시작점을 기준으로 연결됩니다. 영역 크기가 달라지거나 창 기준 좌표가 남아 있으면 이미지·좌표 검토 안내가 표시됩니다.

키 조합은 Windows 가상 키, 문자 입력은 Unicode 이벤트로 전달합니다. 각 입력 직전 대상 창 전경 상태를 확인하고, 전경이 바뀌면 후속 키보드 입력을 중단합니다.

동작 순서는 [MACRO_DESIGN.md](MACRO_DESIGN.md), 진행 중인 검증은 [GOAL.md](GOAL.md)를 참고하세요.

```powershell
npm test
npm run test:overlay-ui
npm run test:keyboard-ui
npm run build
```

키보드 테스트는 별도 수신 창에 실제 Windows 입력을 보냅니다. 테스트 창의 전경 전환이 거부되면 실패하며, 관리자 권한 앱·게임·입력기 조합의 호환성은 별도 확인이 필요합니다.

상단 툴바의 **F7 시작** 버튼(또는 F7 키)으로 선택한 매크로를 시작합니다. 편집 화면의 **진행 오버레이** 버튼을 누르면 전체 단계 목록과 현재 실행 위치·반복 횟수를 별도 항상-위 창에 표시합니다. 클릭 직전에는 클릭 지점에 녹색 링이 0.8초 표시되고, 탐지 성공 프레임은 사용자 데이터 폴더의 `match-shots`에 최대 30장 보관됩니다.
