...
오프라인 라이선스의 서명/MAC/만료/기능 권한 검증, 등록과 갱신, 실행 권한 확인을 지원합니다. 유효한 라이선스가 있으면 선택한 Windows 창에서 키보드/마우스 입력을 실행합니다. 전경 전환에 실패하면 입력을 차단합니다. 시작 단축키는 설정/충돌 검사만 지원하며 아직 등록하지 않습니다. 매크로 import/export는 `.amacro` 패키지로 설정·캡처 이미지를 함께 주고받으며, 가져온 뒤 대상 창 확인과 오버레이 재지정으로 실행합니다. 반복 내부 단계도 워크플로우에서 편집합니다.
...
- `게임 해상도 자동 설정`은 게임 창의 물리 좌표와 Windows DPI 가상화 변환을 함께 읽어 저장 영역을 계산합니다. DPI 미지원 창이 96 DPI를 반환하더라도 확대된 물리 해상도를 그대로 저장하지 않습니다.
- v3에서 영역이 잘못 저장됐다면 대상 게임을 선택하고 `게임 해상도 자동 설정`을 다시 누른 뒤 저장하세요. 기존 기준 이미지가 다른 배율로 캡처됐다면 다시 캡처해야 합니다.
- 순서도는 독립된 캔버스에서 가로·세로 스크롤, 빈 공간 드래그 이동, Ctrl+휠 확대/축소, 전체 보기를 지원합니다. 중첩 분기는 캔버스 안에서 확장됩니다. 실행 구조는 기존의 순차·조건·반복 구조를 유지합니다.
- DPI 변환 회귀 테스트는 150%에서 물리 1920×1440 → 저장 1280×960 → 캡처 1920×1440의 왕복 변환과 DPI 컨텍스트 복구를 검사합니다. 화면 통합 테스트는 5단 중첩 분기의 화면 넘침 방지와 확대/전체 보기를 검사합니다.

## v4.1 반복 탐지 위치 학습

이미지 보관함의 `region`은 최초 캡처 위치로 보존하고, `learned_region`에 최근 성공 위치를 별도로 암호화 저장합니다. 반복 탐지는 최근 위치 → 최초 위치 → 지정 구역 → 전체 영역 순서로 검색합니다. 좌표 힌트만으로 클릭하지 않고 매번 이미지 일치 여부를 확인합니다. 보관함에서 두 위치를 함께 확인할 수 있으며, 재실행과 매크로 내보내기에도 포함됩니다. 게임 대상이나 오버레이가 변경되면 최근 위치는 초기화됩니다.

이미 앞에 있는 게임은 전경 전환을 생략합니다. 그 외에는 최소화 복원과 전경 전환을 최대 3회, 약 1.5초 동안 재시도하며 F9 중지가 가능합니다. Windows가 전경 전환을 거부하면 입력을 보내지 않고 게임 창 직접 선택과 권한 확인을 안내합니다. 자동 해상도 설정은 전경 전환을 요구하지 않으며, 연속 측정한 창 크기가 안정됐을 때 확정합니다.

## v4.2 랜덤 대기와 이미지 배율

순서도에서 `랜덤 대기`를 추가하면 매 실행마다 최소~최대 범위에서 대기 시간을 새로 뽑습니다. 기본값은 1~60초이며 초 단위 정수로 범위를 편집합니다. F8 일시정지와 F9 중지를 지원합니다.

이미지 단계의 `기준 이미지 배율 (%)`은 원본 파일을 보존하며 탐색용 이미지만 25~400%로 조절합니다. 기본은 100%입니다. Windows 150%라는 이유만으로 150을 입력하지 마세요. 캡처가 DPI 보정된 좌표로 정규화되므로 실제 기준 이미지 크기를 비교해야 합니다. 색·글꼴·버튼 선택 상태가 달라졌다면 해당 PC에서 기준 이미지를 다시 캡처하는 편이 정확합니다.
