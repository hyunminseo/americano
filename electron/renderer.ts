...
function update(state: any): void {
  ...
  ($('#runtime-copy') as HTMLElement).textContent = `${run.macroId ? (documentData.macros.find((item) => item.id === run.macroId)?.name || run.macroId) : '실행 대기'}${run.step ? ` · 단계 ${run.step.map((part: string | number) => typeof part === 'number' ? part + 1 : (({ test: '검사', then: 'true', else: 'false' } as Record<string, string>)[part] || part)).join('.')}` : ''} · 반복 ${run.iteration || 0}/${run.iterations || 1} · 탐지 ${run.matched == null ? '대기' : run.matched} · 완료 ${run.completed}회${run.wait_ms != null ? ` · 랜덤 대기 ${(run.wait_ms / 1000).toFixed(2)}초` : ''}`;
...
function fields(action: UiAction): string {
  ...
  return Object.entries(action).filter(([key]) => !['type', 'timeout_ms', 'actions', 'action', 'test', 'then', 'else'].includes(key)).map(([key, value]) => {
    ...
    if (isImageAction && key === 'roi') {
      ...
      return `<label>관심 영역 ROI (이미지 선택 시 자동 입력 · 직접 수정 가능)${['x', 'y', 'width', 'height'].map((part) => `<input data-field="roi.${part}" type="number" min="0" max="1" step="0.01" placeholder="${part}" value="${(roi as unknown as Record<string, unknown>)[part] ?? ''}">`).join('')}</label>`;
    }
