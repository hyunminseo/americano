...
import { prepareWindow } from './windows.js';
import type { ClientRect, WaitControl } from './windows.js';
import type { Region as LogicalRegion } from './macros.js';
...
export function physicalRegion(client: ClientRect, area: LogicalRegion | null): { x: number; y: number; width: number; height: number } {
...
export async function captureTarget(target: TargetWindow, area: LogicalRegion | null, control?: WaitControl): Promise<{ buffer: Buffer; region: { x: number; y: number; width: number; height: number }; dpi: number }> {
  ...
  const buffer = await sharp(image.data, { raw: { width: image.width, height: image.height, channels: image.channels as 1 | 2 | 3 | 4 } }).png().toBuffer();
  return { buffer, region, dpi: prepared.dpi };
}

// 대상 창 전체를 오버레이로 자동 설정한다. client는 네이티브 물리 픽셀이므로
// 96 DPI 논리 좌표로 환산한다. 수동 드래그 없이 시작할 때 사용한다.
export function fullClientOverlay(client: { width: number; height: number; dpi?: number }): LogicalRegion {
