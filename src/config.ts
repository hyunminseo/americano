...
function required<T>(object: Record<string, unknown> | null | undefined, name: string, type: string): T {
  if (typeof object?.[name] !== type) throw new ConfigError(`'${name}' must be ${type}`);
  return (object as Record<string, unknown>)[name] as T;
}

function parseRegion(value: unknown): ConfigRegion {
  if (!value || typeof value !== 'object') throw new ConfigError("'region' must be an object");
  const source = value as Record<string, unknown>;
  const region = {} as ConfigRegion;
  for (const name of ['x', 'y', 'width', 'height'] as const) region[name] = required<number>(source, name, 'number');
...
function parseActions(value: unknown): ConfigAction[] {
  ...
  return (value as Record<string, unknown>[]).map((item) => {
    ...
    const key = required<string>(item, 'key', 'string').toLowerCase();
    if (key.length !== 1 && !ALLOWED_KEYS.has(key) && !key.includes('+')) throw new ConfigError(`unsupported key: ${key}`);
    const delayMs = (item.delay_ms as number | undefined) ?? 0;
...
export function loadConfig(configPath: string): LoadedConfig {
  ...
  try { raw = JSON.parse(fs.readFileSync(absolutePath, 'utf8')) as Record<string, unknown>; }
  catch (error) {
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') throw new ConfigError(`configuration file not found: ${absolutePath}`);
  ...
  const monitor = (raw.monitor as number | undefined) ?? 1;
  const scanIntervalMs = (raw.scan_interval_ms as number | undefined) ?? 100;
  ...
  const targets = (raw.targets as Record<string, unknown>[]).map((item) => {
    ...
    return { id, imagePath, region: parseRegion(item.region), threshold, trigger, cooldownMs, actions: parseActions(item.actions), enabled: (item.enabled as boolean | undefined) ?? true };
  });
