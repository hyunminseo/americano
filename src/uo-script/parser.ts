...
export interface Token {
  type: 'string' | 'number' | 'word' | 'operator';
  value: string;
  column: number;
}
...
export type AstNode =
  ...
  | { type: 'not'; expression: AstNode }
  | { type: 'call'; name: string; args: AstNode[] }
  | { type: 'compare'; operator: string; left: AstNode; right: AstNode }
  | { type: 'and' | 'or'; left: AstNode; right: AstNode }
  | { type: 'command'; name: string; args: unknown[]; line: number }
  | { type: 'if'; branches: { test: AstNode; body: AstNode[] }[]; alternate: AstNode[]; line: number }
  | { type: 'while'; test: AstNode; body: AstNode[]; line: number }
  | { type: 'for'; args: string[]; body: AstNode[]; line: number }
  | { type: 'program'; body: AstNode[]; version: number };
...
function tokenize(source: string, line: number): Token[] {
  ...
  while (index < source.length) {
    ...
    const value = match[0];
    tokens.push({ type: /^-?\d+(?:\.\d+)?$/.test(value) ? 'number' : 'word', value, column });
    index += value.length;
  }
  return tokens;
}
...
function value(tokens: Token[], line: number): AstNode {
...
function primary(tokens: Token[], line: number): AstNode {
  ...
  const left: AstNode = args.length ? { type: 'call', name: (first as { name: string }).name, args } : first;
  if (!tokens[0] || !comparison.has(tokens[0].value)) return left;
  const operator = (tokens.shift() as Token).value;
  return { type: 'compare', operator, left, right: primary(tokens, line) };
}

function expression(tokens: Token[], line: number): AstNode {
  let left = primary(tokens, line);
  while (tokens[0] && ['and', 'or'].includes(tokens[0].value.toLowerCase())) {
    const operator = (tokens.shift() as Token).value.toLowerCase() as 'and' | 'or';
    left = { type: operator, left, right: primary(tokens, line) };
  }
  return left;
}

function command(tokens: Token[], line: number): AstNode {
...
export function parseScript(source: unknown): Extract<AstNode, { type: 'program' }> {
  ...
  let cursor = 0;
  function block(endWords: Set<string> = new Set()): { body: AstNode[]; end: string | null } {
    const body: AstNode[] = [];
    while (cursor < lines.length) {
      ...
      cursor += 1;
      if (head === 'if') {
        const branches = [{ test: expression(tokens.slice(1), current.line), body: null as unknown as AstNode[] }];
        ...
        if (marker.end === 'else') { cursor += 1; alternate = block(new Set(['endif'])).body; marker = { body: [], end: 'endif' }; }
        ...
        cursor += 1;
        body.push({ type: 'while', test: expression(tokens.slice(1), current.line), body: nested.body, line: current.line });
      } else if (head === 'for') {
        const nested = block(new Set(['endfor']));
        if (nested.end !== 'endfor') throw new ScriptSyntaxError('for 블록에 endfor가 필요합니다.', current.line);
        cursor += 1;
        body.push({ type: 'for', args: tokens.slice(1).map((token) => token.value), body: nested.body, line: current.line });
