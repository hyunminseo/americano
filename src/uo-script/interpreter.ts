import { parseScript, AstNode } from './parser.js';

export class ScriptRuntimeError extends Error {}
export class ScriptBreak extends Error {}
export class ScriptContinue extends Error {}
export class ScriptStop extends Error {}

export interface ScriptAdapter {
  value?(name: string, context: ScriptContext): unknown;
  call?(name: string, args: unknown[], context: ScriptContext): unknown;
  command?(name: string, args: unknown[], context: ScriptContext): Promise<void> | void;
}
...
export function createContext({ adapter = {}, control = null, signal = null, maxIterations = 10000, onLog = () => {} }: {
  adapter?: ScriptAdapter;
  control?: ScriptControl | null;
  signal?: AbortSignal | null;
  maxIterations?: number;
...
async function checkpoint(context: ScriptContext): Promise<void> {
...
function value(node: AstNode, context: ScriptContext): unknown {
  ...
  if (node.type === 'compare') {
    const left = value(node.left, context) as number | string;
    const right = value(node.right, context) as number | string;
    switch (node.operator) {
      ...
      case '!=': case '<>': return left !== right;
      default: throw new ScriptRuntimeError(`지원하지 않는 비교 연산자: ${node.operator}`);
    }
  }
  ...
  throw new ScriptRuntimeError(`알 수 없는 표현식: ${(node as AstNode).type}`);
}

function truth(result: unknown): boolean { return Boolean(result); }

async function command(node: Extract<AstNode, { type: 'command' }>, context: ScriptContext): Promise<void> {
  ...
  switch (node.name) {
    case 'pause': {
      ...
      if (context.control?.wait) await context.control.wait(duration);
        ...
        let settled = false;
        const finish = (callback: () => void): void => { if (settled) return; settled = true; context.signal?.removeEventListener('abort', cancel); callback(); };
        const timer = setTimeout(() => finish(resolve), duration);
        const cancel = (): void => finish(() => { clearTimeout(timer); reject(new ScriptRuntimeError('스크립트 실행이 취소되었습니다.')); });
        if (context.signal) {
    ...
    case 'set': {
      ...
      context.variables[args[0] as string] = args[1];
      return;
    }
    case 'setalias': context.aliases[args[0] as string] = args[1]; return;
    case 'unsetalias': delete context.aliases[args[0] as string]; return;
    case 'createlist': context.lists[args[0] as string] ??= []; return;
    case 'clearlist': context.lists[args[0] as string] = []; return;
    case 'pushlist': (context.lists[args[0] as string] ??= []).push(args[1]); return;
    case 'poplist': {
      const list = context.lists[args[0] as string] ?? [];
      if (args[1] === 'front') list.shift();
      else list.pop();
      return;
    }
    case 'createtimer': context.timers[args[0] as string] = Date.now(); return;
    case 'settimer': context.timers[args[0] as string] = Date.now() - Number(args[1] || 0); return;
    case 'removetimer': delete context.timers[args[0] as string]; return;
    default:
...
export async function executeNodes(nodes: AstNode[], context: ScriptContext): Promise<void> {
  for (const node of nodes) {
    ...
    if (node.type === 'if') {
      ...
      await executeNodes(branch ? branch.body : node.alternate, context);
      continue;
    }
    if (node.type === 'while') {
    ...
    throw new ScriptRuntimeError(`실행할 수 없는 AST 노드: ${(node as AstNode).type}`);
  }
}
...
export async function runScript(source: string, options: {
  ...
  maxIterations?: number;
  onLog?: (message: string) => void;
} = {}): Promise<{ ast: Extract<AstNode, { type: 'program' }>; context: ScriptContext }> {
