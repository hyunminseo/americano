...
3. **오버레이 영역 설정**을 누른다. 대상 창을 활성화하고 현재 보이는 클라이언트 영역을 캡처한다.
...
- 탐지 프레임과 캡처 이미지는 96 DPI 기준으로 맞춘 뒤 회색조로 비교한다.
...
마우스 이동은 Windows 좌표 API를 사용하고, 직접 이동이 실패하면 가상 화면 기준 `SendInput` 이동을 시도한다. 실제 커서 위치가 요청 좌표와 같은지 확인한 뒤 클릭한다. 클릭 지점이 다른 창에 가려졌거나 이동 중 취소·전경 변경이 발생하면 클릭하지 않는다. 클릭 이벤트도 Windows `SendInput`으로 전달한다.
