...
현재 코드 기준 문서. 워크플로우 편집, 이미지 포함 import/export, 대상 창 입력 경로를 설명한다.
...
8. 워크플로우에서 조건의 검사·참·거짓 및 반복 안에 동작을 연결한다.
...
워크플로우 캔버스에 위에서 아래로 실행 순서대로 노드를 연결한다. 조건은 참·거짓 레인, 반복은 컨테이너 안에 하위 노드를 담는다.

| 노드 묶음 | 지원 동작 |
| --- | --- |
| 입력 | 키 조합, 문자, 클릭, 마우스 이동, 스크롤 |
| 이미지 탐지 | 이미지 검사, 발견 대기, 발견한 이미지 클릭, 스마트 클릭 |
| 조건과 반복 | 이미지 조건의 참·거짓, 반복, 이미지 재시도, 대기, 중지 |

- 위에서 아래 순서가 실행 순서다. 조건과 반복 안에 노드를 중첩할 수 있다.
- 노드 왼쪽 손잡이를 끌어 원하는 연결 위치에 놓으면 순서가 바뀐다. ↑↓·복제·삭제 버튼과 각 컨테이너 끝의 ＋ 추가로도 편집한다.
- 조건의 검사 칸에는 이미지 검사 하나, 재시도에는 이미지 동작 하나를 연결한다.
- 노드를 클릭하면 오른쪽 인스펙터에서 값을 편집한다. 실행 중인 노드를 강조한다.
- 이미지 노드에서 캡처 이미지를 이름으로 선택한다. 저장된 오버레이가 있으면 그 영역을 우선 사용한다.
- 클릭과 이동은 **오버레이** 또는 **창 영역** 좌표 기준을 선택할 수 있다.
- 저장 형식은 기존 version 2의 `actions`를 유지한다. 예전 매크로도 워크플로우로 불러온다.
...
1. 선택한 매크로의 변경사항을 저장한다. 이미지를 고르지 않은 단계가 있으면 몇 번째 단계인지 알려주고 중단한다.
2. **매크로 내보내기**를 누른다.
3. 이미지 보관함과 중첩된 모든 이미지 노드의 참조를 모은다.
...
4. 각 노드의 이미지 참조를 새 로컬 경로로 연결하고 기존 목록에 추가한다.
5. 대상 프로세스·창 제목을 확인한다. 필요하면 **창 새로고침**으로 다시 선택한다. 대상 창을 고르면 오버레이가 비어 있을 때 전체 창으로 자동 설정된다.
6. **오버레이 영역 설정**으로 새 영역을 지정한다. 저장·재지정하면 테두리가 자동으로 표시된다.
...
- 반복 노드는 내부 동작을 별도로 반복한다. 중지 노드는 전체 실행을 끝낸다.
...
- `npm test`: 스키마, 워크플로우 경로·분기 이동, 패키지 자산 연결·실패 복구, 좌표 재연결, 키 이벤트·부분 실패 정리, 실행 제어 테스트.
- `npm run test:overlay-ui`: 실제 Electron 화면에서 워크플로우 순서 변경, 캡처 저장, 내보내기·가져오기, 오버레이 재지정, 재시작 복원. 캡처와 창 위치는 고정 테스트 자료를 사용한다.
...
- 현재 단위 테스트와 Electron 워크플로우·패키지 통합 테스트가 통과했다. 빌드한 패키지에서 워크플로우 UI, 네이티브 모듈 및 암호화 저장도 확인했다.
...
| 파일 | 역할 |
| --- | --- |
| `electron/renderer.js` | 편집, 캡처, 가져오기·내보내기, 워크플로우 화면 |
